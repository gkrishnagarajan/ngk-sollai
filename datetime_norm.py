...
# -*- coding: utf-8 -*-
"""
Date / time normaliser for NGK Sollai.

Converts Tamil number-words in date and time expressions into digits.
This is FORMATTING, not error correction -- the input may be perfectly
correct Tamil. Keep it as a separate toggleable layer.

Output formats:
    dd.mm.yyyy      full date
    hh:mm           time with minutes (12-hour)
    hh              bare hour
    dd-ஆம் தேதி     ordinal day alone
"""
import re

ENABLED = True

# ---- cardinal number words 1..31 ----
CARD = {
...
# ---- English months in Tamil script ----
MONTH = {
...
def normalise(text):
    text = normalize_case_numbers(text)
    out, changes = text, []
    ...
    # 0) Numeric time: 12 45 மணி/மணிக்கு -> 12:45 மணி/மணிக்கு
    _nt = re.compile(r'(\d{1,2})\s+(\d{2})\s+(மணிக்கு|மணி)')
    def _ntime(m):
        s = '%s:%02d %s' % (m.group(1), int(m.group(2)), m.group(3))
        rec(m.group(0), s, 'time_numeric')
        return s
    out = _nt.sub(_ntime, out)

    # 1) ordinal day + month + 4-digit year  ->  dd.mm.yyyy
    pat = re.compile(r"(?<![\u0B80-\u0BFF])(" + _ORD_ALT + r")\s+(" + _MON_ALT +
                     r")\s+(\d{4})(\s+\u0B85\u0BA9\u0BCD\u0BB1\u0BC1)?")
    def _full(m):
        d = "%02d.%02d.%s" % (ORD[m.group(1)], MONTH[m.group(2)], m.group(3))
        s = d + "-\u0B86\u0BAE\u0BCD \u0BA4\u0BC7\u0BA4\u0BBF\u0BAF\u0BA9\u0BCD\u0BB1\u0BC1"
        rec(m.group(0), s, "date"); return s
    out = pat.sub(_full, out)

    # digits already present: 12.03.2024 அன்று -> 12.03.2024-ஆம் தேதியன்று
    pat = re.compile(r"(\d{2}\.\d{2}\.\d{4})\s+\u0B85\u0BA9\u0BCD\u0BB1\u0BC1")
    def _dig(m):
        s = m.group(1) + "-\u0B86\u0BAE\u0BCD \u0BA4\u0BC7\u0BA4\u0BBF\u0BAF\u0BA9\u0BCD\u0BB1\u0BC1"
        rec(m.group(0), s, "date"); return s
    out = pat.sub(_dig, out)

    # 1b) numeric variants -> dd.mm.yyyy-ஆம் தேதியன்று
    #     handles  27 4/2019 ஆம் தேதி | 27/4/2019 | 27-4-2019 | 27.4.2019
    SUF = " ஆம் தேதியன்று"
    TAIL = r"(?:\s*[-\u2013]?\s*(?:\u0B86\u0BAE\u0BCD|\u0BAE\u0BCD))?(?:\s*\u0BA4\u0BC7\u0BA4\u0BBF(?:\u0BAF\u0BA9\u0BCD\u0BB1\u0BC1|\u0BAF\u0BBF\u0BB2\u0BCD|\u0B87\u0BB2\u0BCD)?)?(?:\s*\u0B85\u0BA9\u0BCD\u0BB1\u0BC1)?"
    pat = re.compile(r"(?<![\d/])(\d{1,2})[\s./-]+(\d{1,2})[\s/.-]+(\d{4})" + TAIL)
    def _num(m):
        full = m.group(0)
        # Skip if suffix already present after match
        end = m.end()
        after = out[end:end+25]
        if "தேதியன்று" in after or after.startswith("-ஆம்") or "-ம் தேதி" in after or after.startswith("-ம்"):
            return full
        # Skip if match itself already has formatted suffix
        if "தேதி" in full and ("-ம்" in full or "-ஆம்" in full):
            return full
    ...
    SUF2 = " ஆம் தேதி"
    TL = r"(?:\s*[-\u2013]?\s*(?:\u0B86\u0BAE\u0BCD|\u0BAE\u0BCD))?(?:\s*\u0BA4\u0BC7\u0BA4\u0BBF(?:\u0BAF\u0BA9\u0BCD\u0BB1\u0BC1|\u0BAF\u0BBF\u0BB2\u0BCD|\u0BAF\u0BBF\u0BB2\u0BC1\u0BAE\u0BCD)?)?(?:\s*\u0B85\u0BA9\u0BCD\u0BB1\u0BC1)?"
    pat = re.compile(r"(?<!\d)(\d{1,2}|" + _CW + r")[\s,]+?(\d{1,2}|" + _CW + r")[\s,]+?(\d{4})" + TL)
    def _mix(m):
        a, b = m.group(1), m.group(2)
        d = int(a) if a.isdigit() else CARD.get(a, 0)
        mo = int(b) if b.isdigit() else CARD.get(b, 0)
        if not d or not mo or mo > 12:
            return m.group(0)
        # Skip if date-suffix already present after match
        _after = out[m.end():m.end()+25]
        if ("தேதியன்று" in _after
                or _after.startswith("-ஆம்")
                or _after.startswith("-ம்")):
            return m.group(0)
        s2 = "%02d.%02d.%s%s" % (d, mo, m.group(3), SUF2)
        rec(m.group(0).strip(), s2, "date"); return s2
    out = pat.sub(_mix, out)

    # 2) time with minutes: <card> மணி <card> நிமிடம்  ->  h:mm
    pat = re.compile(r"(?<![\u0B80-\u0BFF])(" + _CARD_ALT + r")\s+" + MANI +
                     r"\s+(" + _CARD_ALT + r")\s+" + NIMIDAM)
    def _hm(m):
        s = "%d:%02d" % (CARD[m.group(1)], CARD[m.group(2)])
...
def normalize_case_numbers(text):
    """குற்ற எண். 190, 2021 → குற்ற எண். 190/2021"""
    import re
    text = re.sub(
        r'(குற்ற\s+எண்\.+\s{0,3})(\d{1,6})\s*[,\s]\s*((19|20)\d{2})\b',
        r'\1\2/\3',
        text
    )
    return text
