...
Output formats:
    dd.mm.yyyy-ஆம் தேதியன்று   full date
    hh.mm மணி                  time with minutes (dot separator)
    hh மணி                     bare hour
    dd-ஆம் தேதி                ordinal day alone

Changes from original:
    - Added numeric time with space: 12 45 மணி -> 12.45 மணி
    - Added numeric time with space: 12 55 மணி -> 12.55 மணி
    - Added word-number date: கடந்த இரண்டு 10 2022 -> 10.02.2022-ஆம் தேதியன்று
"""
import re
...
# ---- Tamil month word numbers (for கடந்த இரண்டு 10 2022 type) ----
MONTH_WORDS = {
...
_MWORD_ALT = "|".join(sorted(MONTH_WORDS, key=len, reverse=True))
...
def normalise(text):
    ...
    # 0) NEW: numeric time with space separator
    #    பகல் 12 45 மணி  ->  பகல் 12.45 மணி
    #    மாலை 6 30 மணி   ->  மாலை 6.30 மணி
    def _num_time(m):
    ...
    # 0b) NEW: கடந்த <month_word> <day_digit> <year>
    #     கடந்த இரண்டு 10 2022 -> 10.02.2022-ஆம் தேதியன்று
    #     i.e. "கடந்த" + month-word + day-number + year
    pat_word_date = re.compile(
        r'கடந்த\s+(' + _MWORD_ALT + r')\s+(\d{1,2})\s+(\d{4})(?:\s*-?ம்)?(?:\s*தேதி)?'
    )
    def _word_date(m):
        mo = MONTH_WORDS[m.group(1)]
        d  = int(m.group(2))
        yr = m.group(3)
        s  = "கடந்த %02d.%02d.%s%s" % (d, mo, yr, SUF)
